...
try:
    ...
    pi_gen = foo(5)
    my_pi.mth(pi_gen.__next__())  # Iterating over the not iterable object
    ...
    my_pi.mth(
        pi_gen.__next__()
    )  # Iterating over the not iterable object, not even the one we just tried to create
    my_pi.mth(pi_gen.__next__())
    my_pi.mth(pi_gen.__next__())
...
enumerate(my_pi)  # Again iterating over not iterable object
print("my second pi")
enumerate(my_pi_2)  # Same as the comment above
